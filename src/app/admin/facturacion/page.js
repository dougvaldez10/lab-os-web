...
export default function BillingPanel() {
  ...
  const printReceipt = (calc, customCaseObj = null) => {
    ...
    // Construimos un objeto compatible con el formato que usa printThermalReceipt
    // (el mismo que ya funciona correctamente en "Casos Pendientes"), para que
    // el recibo se vea IDÉNTICO sin importar desde qué pantalla se imprima.
    const casoParaRecibo = {
      codigo: activeCase.id,
      paciente: activeCase.patient,
      doctor: activeCase.doctor,
      clientes: { nombre: activeCase.doctor },
      casos_detalle: (activeCase.items || []).map(it => ({
        unidades: it.unidades,
        producto: it.producto,
        dientes: it.dientes,
        subtotal: Number(it.unidades) * Number(it.precio_unitario)
      })),
      total_caso: calc.total,
      descuento: calc.discountAmount,
      saldo_pendiente: 0
    };

    printThermalReceipt({ caso: casoParaRecibo, abono: 0 });
  };
